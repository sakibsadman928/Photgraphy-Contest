...
const NEXT_ACTION: Partial<
  Record<Contest["status"], { label: string; endpoint: string; method: "post" }>
> = {
  ...
  registration_closed: {
    label: "Open Round 1",
    endpoint: "round1/open",
    method: "post",
  },
  round1_open: {
    label: "Close Round 1",
    endpoint: "round1/close",
    method: "post",
  },
...
function ManageContestContent() {
  ...
  const load = useCallback(async () => {
    const data = await api.get<{ contest: Contest; registeredCount: number }>(
      `/contests/${id}`,
    );
    ...
    const round =
      data.contest.status === "round1_closed"
        ? "round1"
        : data.contest.status === "final_closed"
          ? "final"
          : null;
    if (round) {
      const progressData = await api.get<JudgingProgress>(
        `/contests/${id}/${round}/progress`,
      );
  ...
  const runAction = async (
    ...
    try {
      ...
      if (
        ...
        err.statusCode === 409 &&
        (err.details as any)?.tiedSubmissionIds
      ) {
        const ids: string[] = (err.details as any).tiedSubmissionIds;
        const round = currentRound ?? "round1";
        const allSubs = await api.get<{ submissions: SubmissionRow[] }>(
          `/contests/${id}/submissions/${round}/all`,
        );
        setTiedSubmissions(
          allSubs.submissions.filter((s) => ids.includes(s._id)),
        );
  ...
  const handleResolveTie = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Tie resolved. You can try publishing again.",
      });
    } catch (err) {
      setMessage({
        type: "error",
  ...
  const handleReplaceJudge = async (judgeId: string) => {
    ...
    await runAction(`judges/${judgeId}/replace`, "post", {
      newJudgeId: replacementJudgeId,
    });
  ...
  const assignedJudgeIds = new Set(
    contest.judges
      .filter((j) => j.active)
      .map((j) => (typeof j.judge === "string" ? j.judge : j.judge._id)),
  );
  const availableJudgesToAdd = allJudges.filter(
    (j) => !assignedJudgeIds.has(j._id),
  );
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {message && (
        <p
          className={`mt-4 text-sm ${message.type === "error" ? "text-accent-text" : "text-teal"}`}
        >
          {message.text}
        </p>
      )}
      ...
      <Card>
        <p className="text-sm text-ink-muted">
          ...
          <span className="font-mono text-ink">{registeredCount}</span>
        </p>

        {contest.status === "cancelled" && (
          <p className="mt-3 text-sm text-accent-text">
            Cancelled: {contest.cancelReason}
          </p>
        )}
        ...
        {progress && currentRound && (
          <div className="mt-4 border-t border-hairline pt-4">
            <p className="text-xs uppercase tracking-wide text-ink-muted">
              Judging progress (
              {currentRound === "round1" ? "Round 1" : "Final"})
            </p>
            ...
            {!progress.complete && (
              <p className="mt-2 text-xs text-amber">
                Publishing is blocked until every judge finishes. Replace an
                unresponsive judge below if needed.
              </p>
            )}
          </div>
        )}
        ...
        {tiedSubmissions && (
          <div className="mt-4 rounded-xl border border-amber/30 bg-amber-light p-4">
            <p className="text-sm font-medium text-ink">
              A tie needs your decision — pick the winning photo:
            </p>
        ...
        <div className="mt-5 flex gap-3">
          {nextAction && (
            <Button
              onClick={() => runAction(nextAction.endpoint)}
              loading={busy}
            >
              {nextAction.label}
            </Button>
          ...
          {publishRoundAction && (
            <Button
              onClick={() => runAction(publishRoundAction.endpoint)}
              loading={busy}
            >
          ...
          {contest.status === "completed" && (
            <p className="text-sm text-teal">Contest completed.</p>
          )}
        </div>
      </Card>
          ...
          {contest.judges.map((j, i) => {
            ...
            return (
              ...
              >
                <div>
                  <p
                    className={`text-sm ${j.active ? "text-ink" : "text-ink-muted line-through"}`}
                  >
                    {judgeObj?.name ??
                      (typeof j.judge === "string" ? j.judge : "")}
                  </p>
                  {!j.active && (
                    <p className="text-xs text-ink-muted">Replaced</p>
                  )}
                </div>
                {j.active &&
                      ...
                      {replacingJudgeId === (judgeObj?._id ?? j.judge) ? (
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            onChange={(e) =>
                              setReplacementJudgeId(e.target.value)
                            }
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleReplaceJudge(
                                judgeObj?._id ?? (j.judge as string),
                              )
                            }
                            loading={busy}
                          >
